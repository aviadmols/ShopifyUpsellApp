...
<div class="space-y-4">
  ...
  @if($payload && ($diagnosis || $error))
    @if(!empty($payload['discrepancy_possible_cause']))
      <p class="text-sm text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2">{{ e($payload['discrepancy_possible_cause']) }}</p>
    @endif
