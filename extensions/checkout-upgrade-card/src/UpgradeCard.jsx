...
function getContextPayloadFromApi(api) {
  ...
  try {
    ...
    if (addr) {
      ...
      if (raw && typeof raw === 'object' && raw.phone) {
        payload.phone = String(raw.phone ?? '');
      }
    }
  } catch (_) {}
  try {
    const identity = api?.buyerIdentity;
    if (identity && typeof identity === 'object') {
      const unwrap = (v) => {
        if (v == null) return v;
        if (typeof v === 'string') return v;
        if (typeof v?.current === 'function') return v.current();
        if (v?.value !== undefined) return v.value;
        return v?.current;
      };
      const e = unwrap(identity.email);
      const p = unwrap(identity.phone);
      if (e !== undefined && e !== null) payload.email = String(e);
      if (p !== undefined && p !== null) payload.phone = String(p);
    }
  } catch (_) {}
...
function buildLineItemsForPixel(lineItems) {
...
function buildPixelPayload(lineItems, attributes, subtotalMoney, blockId, shop, sessionKey, contextPayload) {
  const client_id = String(
    attributes?._zyxel_user_id || attributes?._axon_client_id || attributes?.igId || ''
  );
  ...
  const now = new Date();
  const event_timestamp = now.toISOString();
  const page_timestamp = now.getTime();
  return {
    client_id,
    checkout_token: sessionKey ?? '',
    total_price,
    currency,
    line_items,
    line_items_count: line_items.length,
    email: contextPayload?.email ?? '',
    phone: contextPayload?.phone ?? '',
    customer_id: null,
    event_timestamp,
    page_timestamp,
...
function sendPixelEvent(eventName, payload) {
  if (!eventName || typeof eventName !== 'string') return;
  const event_value = JSON.stringify({
    client_id: payload.client_id,
    checkout_token: payload.checkout_token ?? payload.session_key ?? '',
...
function UpgradeCard() {
  ...
  const getLinesFromApi = useCallback(() => {
    const raw = api?.lines;
    if (!raw) return [];
    if (Array.isArray(raw)) return raw;
    if (typeof raw?.value !== 'undefined' && Array.isArray(raw.value)) return raw.value;
    if (typeof raw?.current === 'function') return Array.isArray(raw.current()) ? raw.current() : [];
    if (Array.isArray(raw?.current)) return raw.current;
    return [];
  }, [api?.lines]);
  const lineItems = getLinesFromApi();
  ...
  }, [api, apiUrl, secret, shop, blockId, sessionKey, subtotalMoney?.amount, subtotalMoney?.currencyCode, lineItems, JSON.stringify(normalizeLineItemsForApi(lineItems))]);
  ...
  const runActions = useCallback(async () => {
    ...
    const attrs = getCheckoutAttributes(api);
    const ctx = getContextPayloadFromApi(api);
    const linesArray = getLinesFromApi();
    ...
    const resolveLineIdForUpdate = (action, currentLines) => {
    ...
    try {
      for (let i = 0; i < actions.length; i++) {
        ...
        const currentLines = getLinesFromApi();
        if (type === 'updateCartLine') {
          const lineId = resolveLineIdForUpdate(action, currentLines) || action?.lineId;
          if (lineId) {
            ...
            if (action && 'sellingPlanId' in action) {
              ...
              if (action.sellingPlanId != null) {
                ...
                if (line) change.attributes = buildAttributesWithUpgradeMarker(line);
              }
            }
  ...
  }, [payload?.actions, payload?.mode, cartEditable, applyCartLinesChange, api, getLinesFromApi, apiUrl, secret, blockId, sessionKey, shop, subtotalMoney]);

  if (loading && (payload === null || payload === undefined)) {
    return null;
  }
